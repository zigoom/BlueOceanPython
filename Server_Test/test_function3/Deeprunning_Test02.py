...
def make_model(ticker='005930', stt='2010-01-01', end='2023-08-25'):
    # 오늘 날자 받기
    ...
    foo['real+1'] = foo['real+1'].fillna(method='bfill')  # NaN 값을 이전 행의 값으로 대체
    corr_matrix = foo[['real+1', 'pred']].corr()
    print(corr_matrix)

    # MSE, RMSE, MAE 계산
    test_mse = mean_squared_error(foo['real+1'], foo['pred'])
    test_rmse = mean_squared_error(foo['real+1'], foo['pred'], squared=False)
    test_mae = mean_absolute_error(foo['real+1'], foo['pred'])
